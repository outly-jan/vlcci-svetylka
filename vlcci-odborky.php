...
(function(){
  ...
  document.addEventListener("submit",function(e){
	...
	private function app_do_hromadne_stezka( string $base ): void {
		global $wpdb;
		$kompetence_id = intval( $_POST['kompetence_id'] ?? 0 );
		$raw_ids       = array_map( 'intval', (array) ( $_POST['sestka_ids'] ?? [] ) );
		$sestka_ids    = array_filter( $raw_ids, fn($id) => $id > 0 && $this->can_edit_sestka( $id ) );
		if ( empty( $sestka_ids ) ) {
			$this->app_set_flash( 'Nevybrána žádná šestka.', 'error' );
	...
	private function app_page_zapsat_stezku(): void {
		...
		foreach ( $edit_sestky as $s ) {
			...
			foreach ( $deti as $d ) {
				...
				$cls          = $bez_novacka ? ' voa-zapsat-item--locked' : '';
	...
	private function app_page_dashboard(): void {
		...
		foreach ( $sestky as $s ) {
			...
			if ( $can_edit ) {
				...
				echo '<a href="' . esc_url( $this->app_url( 'plneni', [ 'sestka_id' => $s->id ] ) ) . '" class="voa-btn voa-btn-sm voa-btn-white" style="border-color:#f59e0b;color:#92400e">✏️ Odborky</a>';
				if ( $s->typ === 'vlcata' ) echo '<a href="' . esc_url( $this->app_url( 'stezky', [ 'sestka_id' => $s->id ] ) ) . '" class="voa-btn voa-btn-sm voa-btn-white" style="border-color:#3b82f6;color:#1e3a8a">🗺️ Stezky</a>';
				echo '</div>';
			}
			echo '</div>';
			...
			if ( empty( $deti ) ) {
				...
				foreach ( $deti as $d ) {
					...
					if ( $splneno || $rozp ) {
						echo '<div class="voa-child-stat-group">';
						echo '<div class="voa-child-stat-label">Odborky</div>';
					...
					if ( isset( $stezky_done[ $d->id ] ) ) {
						$sd = $stezky_done[ $d->id ];
						echo '<div class="voa-child-stat-group">';
						echo '<div class="voa-child-stat-label">Kompetence</div>';
...
.voa-menu-row--odborky a{border-top:3px solid #f59e0b}
...
.voa-child-stat-label{font-size:10px;color:#888;margin-bottom:2px;font-weight:600;text-transform:uppercase;letter-spacing:.4px}
...
.voa-po-odb-img{width:48px;height:48px;object-fit:contain}
...
.voa-sprava-row{display:flex;flex-wrap:wrap;align-items:center;gap:8px;padding:7px 10px;border-bottom:1px solid #eee}
.voa-sprava-row:last-child{border-bottom:none}
.voa-sprava-info{flex:0 0 260px;min-width:180px}
.voa-sprava-form{display:flex;flex-wrap:wrap;align-items:center;gap:6px;flex:1}
.voa-sprava-check{font-size:12px;white-space:nowrap;display:flex;align-items:center;gap:3px}
.voa-sprava-garant{display:flex;align-items:center;gap:4px}
/* Po dětech */
